...
def generate_data2(
    ...
    while True:
        ...
        for i in range(batch_size):
            lays.append(generate_random_layout(S, H, randint(S*2,S*(H-2))))
...
def reinforcement_training(model: Model, S: int, H: int, MPC: int , 
                           sample_size: int = 50000, iter: int = 5, max_steps: int = 30, epochs: int = 10, 
                           batch_size: int = 20, verbose: bool = True, perms_by_layout: int = 1, learning:int = 90) -> None:
    
    if S is None or H is None or model is None: raise ValueError("S or H has no value")
    ...
    for i in range(iter):
        ...
        if verbose: print('')
        if results_model > learning: break
